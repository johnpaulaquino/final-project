    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          <svg className="w-5 h-5 text-[#800000]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      {savedAddresses.length === 0 ? (
        <div onClick={onOpenModal} className="w-full border-2 border-dashed border-gray-200 rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:border-[#800000] hover:bg-red-50 transition-colors">
          ...
          <p className="text-gray-500 text-sm">You haven't saved an address yet.</p>
        </div>
      ) : isExpanded ? (
            ...
            return (
              ...
              >
                <div className={`mt-1 w-[14px] h-[14px] rounded-full border flex items-center justify-center flex-shrink-0 ${
                  ...
                  {isSelected && <div className="w-[6px] h-[6px] rounded-full bg-[#800000]"></div>}
                </div>

                <div className="flex-grow">
                  <div className="flex items-center flex-wrap gap-2 sm:gap-3 mb-1">
                    <span className={`font-bold ${isSelected ? 'text-gray-900' : 'text-gray-700'}`}>{address.name}</span>
                    <span className="text-gray-500 text-sm">{address.phone}</span>
                    {isSelected && (
                      <span className="bg-[#E2E8F0] text-slate-600 text-[9px] font-bold px-2 py-0.5 rounded tracking-wider">DEFAULT</span>
                    )}
                  </div>
                  <p className="text-gray-500 text-sm leading-relaxed mt-1">{address.fullAddress}</p>
                </div>
              </div>
            );
          })}
          ...
          {activeAddress && (
            <>
              <div className="flex items-center gap-3 mb-1">
                ...
                <span className="text-gray-500 text-sm">{activeAddress.phone}</span>
              </div>
              <p className="text-gray-500 text-sm leading-relaxed mt-2">{activeAddress.fullAddress}</p>
            </>
          )}
        </div>
      )}
