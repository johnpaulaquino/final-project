...
import ConfirmationModal from "@/app/customer/components/ConfirmationModal"; // Adjust path if needed
...
export default function Carousel() {
  ...
  const [bannerToDelete, setBannerToDelete] = useState<string | number | null>(
    null,
  );
  ...
  // The handler for the Confirmation Modal
    ...
    try {
      ...
      setStatusMessage({
        ...
        text:
          ...
          "Failed to delete banner from server.",
      });
    } finally {
      setIsDeleting(false);
      setBannerToDelete(null); // Close the modal
    }
  };
            ...
            async (blob) => {
              ...
              try {
                ...
                setStatusMessage({
                  ...
                  text:
                    error?.response?.data?.detail ||
                    "Failed to upload banner to server.",
                });
              } finally {
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <label
          className={`cursor-pointer transition-all active:scale-95 text-white text-sm font-bold py-2.5 px-4 rounded-[10px] shadow-sm flex items-center gap-2 h-[42px] ${
            isUploading
              ? "bg-gray-400 cursor-not-allowed"
              : "bg-[#0B1527] hover:bg-gray-800"
          }`}
        >
          {isUploading ? (
            <svg
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {banners.map((banner, index) => (
              <div
                key={banner.id}
                className="group border border-gray-100 rounded-[10px] p-3 bg-white hover:border-gray-300 transition-colors"
              >
