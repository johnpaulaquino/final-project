...
export default function EditProductModal({ product, isOpen, onClose, onSave }: EditModalProps) {
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-8">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h3 className="text-2xl font-black text-[#0B1527]">Edit Product</h3>
              <p className="text-sm text-gray-500">Update details for {product?.name}</p>
            </div>
            <button onClick={onClose} className="p-2 bg-gray-100 rounded-full text-gray-400 hover:text-gray-900 transition-colors">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-gray-400 uppercase ml-1">Stock Level</label>
                <input 
                  type="number" 
                  value={formData.stock ?? 0} // Added ?? 0 for numbers
                  onChange={(e) => setFormData({...formData, stock: parseInt(e.target.value) || 0})}
                  className="p-3.5 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:border-[#800000] font-medium"
                />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-gray-400 uppercase ml-1">Image</label>
                <input 
                  type="file" 
            ...
            <div className="flex gap-3 pt-4">
              <button type="button" onClick={onClose} className="flex-1 py-4 bg-gray-100 text-gray-600 font-bold rounded-2xl hover:bg-gray-200 transition-all">
                Cancel
              </button>
              <button type="submit" className="flex-1 py-4 bg-[#800000] text-white font-bold rounded-2xl hover:bg-red-900 shadow-lg shadow-red-200 transition-all active:scale-95">
                Save Changes
              </button>
            </div>
          </form>
