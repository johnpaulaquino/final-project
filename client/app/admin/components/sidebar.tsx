...
export default function Sidebar({ activeTab, setActiveTab }: SidebarProps) {
  ...
  // 🚀 MODIFIED: Renamed and added loading state handling
  const handleConfirmLogout = async () => {
    setIsLoggingOut(true);
    try {
      await apiClient.logout();
      // If your apiClient doesn't automatically redirect, you might need:
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-1.5 custom-scrollbar">
          ...
          <div>
            ...
            >
              ...
              <svg
                className={`w-4 h-4 transition-transform text-gray-400 ${isProductsOpen ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            {isProductsOpen && (
              <div className="flex flex-col gap-1 pl-[44px] pr-4 py-2 relative before:content-[''] before:absolute before:left-[23px] before:top-2 before:bottom-2 before:w-[1px] before:bg-gray-200">
                <button
                  ...
                  className={`py-2 text-sm text-left transition-colors ${activeTab === "Inventory List" ? "text-[#800000] font-bold" : "text-gray-500 hover:text-gray-900"}`}
                >
                  Inventory List
                </button>
                <button
                  onClick={() => setActiveTab("Add Item")}
                  className={`py-2 text-sm text-left transition-colors ${activeTab === "Add Item" ? "text-[#800000] font-bold" : "text-gray-500 hover:text-gray-900"}`}
                >
                  Add Item
                </button>
                <button
                  onClick={() => setActiveTab("Categories")}
                  className={`py-2 text-sm text-left transition-colors ${activeTab === "Categories" ? "text-[#800000] font-bold" : "text-gray-500 hover:text-gray-900"}`}
                >
                  Categories
                </button>
                <button
                  onClick={() => setActiveTab("Carousel")}
                  className={`py-2 text-sm text-left transition-colors ${activeTab === "Carousel" ? "text-[#800000] font-bold" : "text-gray-500 hover:text-gray-900"}`}
                >
                  Carousel
                </button>
              </div>
          ...
          >
            ...
            <svg
              className={`w-4 h-4 transition-transform text-gray-400 ${isCustomersOpen ? "rotate-180" : ""}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {isCustomersOpen && (
            <div className="flex flex-col gap-1 pl-[44px] pr-4 py-2 relative before:content-[''] before:absolute before:left-[23px] before:top-2 before:bottom-2 before:w-[1px] before:bg-gray-200">
              <button
                ...
                className={`py-2 text-sm text-left transition-colors ${activeTab === "Customer List" ? "text-[#800000] font-bold" : "text-gray-500 hover:text-gray-900"}`}
              >
                Customer List
              </button>
              <button
                onClick={() => setActiveTab("Sales History")}
                className={`py-2 text-sm text-left transition-colors ${activeTab === "Sales History" ? "text-[#800000] font-bold" : "text-gray-500 hover:text-gray-900"}`}
              >
                Sales History
              </button>
              <button
        ...
        <div className="p-4 mb-2">
          <div className="flex items-center justify-between p-3 rounded-2xl bg-white border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] cursor-pointer hover:border-gray-200 transition-colors">
            ...
            <button
              ...
              onClick={() => setIsLogoutModalOpen(true)} // 🚀 Now opens the modal
            >
              <svg
                className="w-5 h-5"
                fill="none"
      ...
      <ConfirmationModal
        ...
        confirmColorClass="bg-[#800000] hover:bg-[#600000]"
      />
    </>
  );
}
